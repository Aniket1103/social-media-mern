...
const MediaCard = ({ post }) => {
  ...
  const baseUrl = process.env.NODE_ENV === "development" ? "http://localhost:4000" : "https://socio-vibe-server.onrender.com";
    ...
    <View style={ styles.card }>
      <TouchableOpacity key={_id} style={{flex : 1, borderRadius: 10, overflow: "hidden"}} onPress={ () => handleCardPress(post) }>
        <Image
          ...
          source={{ uri : mediaUrl}}
        />
        <TouchableOpacity style={styles.heartIconContainer} onPress={() => setLike(!like)}>
        {/* <Icon name={`heart`} size={23} color="#fff"/> */}
        <View style={{position: "absolute", top: 0, left: 0, right: 0, bottom: 0, justifyContent: "center", alignItems: "center"}}>
          <Icon name={!like ? 'heart' : 'heart-fill'} size={23} color={!like ? "#fff" : "#f44336"}/>
        </View>
        </TouchableOpacity>
